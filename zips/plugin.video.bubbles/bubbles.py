...
if action == None or action == 'home':
	...
	sources(type = type, kids = kids).play(title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, meta, select, preset = preset, seasoncount = seasoncount)

elif action == 'playExact':
	from resources.lib.sources import sources
	terms = params.get('terms')
	sources(type = type, kids = kids).playExact(terms)
	...
	sources(type = type, kids = kids).alterSources(url)
...
# [BUBBLESCODE]
	...
	from resources.lib.extensions import interface
	interface.Loader.show()
	sources(type = type, kids = kids).presetSources(url)
