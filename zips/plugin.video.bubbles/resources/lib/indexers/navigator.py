...
class navigator:
	...
	def addDirectoryItem(self, name, query, thumb, icon, queue = False, isAction = True, isFolder = True, iconSpecial = interface.Icon.SpecialNone, shortcut = True):
	...
	def informationNavigator(self):
		...
		self.addDirectoryItem(35109, 'legalDisclaimer', 'legal.png', 'DefaultAddonProgram.png')
	...
	def donationsNavigator(self):
		...
		self.addDirectoryItem(interface.Format.color('Decred', '2ED8A3'), 'donationsCrypto&currency=%s' % tools.Donations.CurrencyDecred, 'donationsdecred.png', 'DefaultAddonProgram.png', iconSpecial = interface.Icon.SpecialDonations)
		...
		self.addDirectoryItem(interface.Format.color('Aragon', '00B8E4'), 'donationsCrypto&currency=%s' % tools.Donations.CurrencyAragon, 'donationsaragon.png', 'DefaultAddonProgram.png', iconSpecial = interface.Icon.SpecialDonations)
		self.addDirectoryItem(interface.Format.color('EOS', 'F3F3F3'), 'donationsCrypto&currency=%s' % tools.Donations.CurrencyEos, 'donationseos.png', 'DefaultAddonProgram.png', iconSpecial = interface.Icon.SpecialDonations)
		self.addDirectoryItem(interface.Format.color('Gnosis', '00A6C4'), 'donationsCrypto&currency=%s' % tools.Donations.CurrencyGnosis, 'donationsgnosis.png', 'DefaultAddonProgram.png', iconSpecial = interface.Icon.SpecialDonations)
		self.endDirectory()
	...
	def downloadsBrowse(self, type = None, error = False):
		if error:
			...
			for item in [('movies', 32001), ('shows', 32002), ('documentaries', 33491), ('shorts', 33471)]:
				path = control.setting('downloads.%s.path.%s' % (type, item[0]))
				if tools.File.exists(path):
					action = path
					actionIs = False
				else:
					action = 'downloadsBrowse&downloadType=%s&downloadError=%d' % (type, int(True))
					actionIs = True
				self.addDirectoryItem(item[1], action, 'downloads%s.png' % item[0], 'DefaultAddonProgram.png', isAction = actionIs)
			self.endDirectory()
	...
	def premiumizeDownloadsNavigator(self, lite = False):
		...
		if not lite:
			if valid: self.addDirectoryItem(33013, 'premiumizeClear', 'downloadsclear.png', 'DefaultAddonProgram.png', isAction = True, isFolder = False)
	...
	def realdebridDownloadsNavigator(self, lite = False):
		...
		if not lite:
			if valid: self.addDirectoryItem(33013, 'realdebridClear', 'downloadsclear.png', 'DefaultAddonProgram.png', isAction = True, isFolder = False)
