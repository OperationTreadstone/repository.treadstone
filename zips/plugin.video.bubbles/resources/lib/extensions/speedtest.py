...
class SpeedTester(object):
	...
	def performance(self, test = True):
		...
		if self.mLatency == None or self.mLatency <= 0 or self.mDownload == None or self.mDownload <= 0:
			performance = self.PerformanceMedium
		elif self.mDownload <= self.PerformanceSpeedSlow:
			performance = self.PerformanceSlow
		elif self.mDownload <= self.PerformanceSpeedMedium:
			if self.mLatency >= self.PerformanceLatencySlow:
				performance = self.PerformanceSlow
			else:
				performance = self.PerformanceMedium
		else:
			if self.mLatency >= self.PerformanceLatencySlow:
				performance = self.PerformanceSlow
			elif self.mLatency >= self.PerformanceLatencySlow:
				performance = self.PerformanceMedium
			else:
				performance = self.PerformanceFast
	...
	def _informationInternal(self, service = ServiceNone, selection = SelectionAverage):
	...
	@classmethod
	def _information(self, service = ServiceNone, selection = SelectionAverage, networkInformation = False):
		try:
			information = network.Networker.information()
			try: lottery = tools.Settings.getInteger('general.statistics.lottery.time.claim')
			except: lottery = 0
			data = {
				'type' : self.ApiType,
				'action' : self.ApiActionRetrieve,
				'selection' : selection,
			...
			if not service == self.ServiceNone:
				data['service'] = service

			result = client.request(self._linkApi(), post = urllib.urlencode(data))
			result = tools.Converter.jsonFrom(result)['data'][selection]
			...
			if self.mInformation:
				...
				for i in range(len(titles)):
					...
					if areas[i] in networkInformation:
						...
						if not location: location = self.UnknownCapitalize
	...
	@classmethod
	def _linkApi(self):
		return tools.Converter.base64From(tools.Settings.getString('link.api', raw = True), iterations = 3)
	...
	def _shareBackground(self):
		try:
			if tools.Settings.getBoolean('general.statistics.sharing'):
				...
				id = tools.Hash.sha256(tools.Hash.random() + information['connection']['address'] + str(timestamp))
				...
				data = {
					'type' : self.ApiType,
					'action' : self.ApiActionAdd,
					'data' : json.dumps(information), # Dump, otherwise urlencode uses single quotes and uses UTF instead of ASCII.
					'lottery' : tools.Settings.getInteger('general.statistics.lottery.time.claim'),
				}

				result = client.request(self._linkApi(), post = urllib.urlencode(data))
				result = tools.Converter.jsonFrom(result)['data']['lottery']
				if result['won']:
	...
	def voucher(self):
		...
		if not lotteryType == None and not lotteryType == '' and not lotteryVoucher == None and not lotteryVoucher == '':
			lotteryDescription = tools.Settings.getString('general.statistics.lottery.description')
			lotteryInstruction = tools.Settings.getString('general.statistics.lottery.instruction')
			lotteryTimeClaim = tools.Settings.getInteger('general.statistics.lottery.time.claim')
			lotteryTimeExpiry = tools.Settings.getInteger('general.statistics.lottery.time.expiry')
			lotteryTimeDuration = tools.Settings.getInteger('general.statistics.lottery.time.duration')

			indent = '     '
			message = interface.Format.bold(interface.Translation.string(33875)) + interface.Format.newline()
			message += '%s%s: %s' % (indent, interface.Translation.string(33343), interface.Format.bold(lotteryType)) + interface.Format.newline()
			message += '%s%s: %s' % (indent, interface.Translation.string(33876), interface.Format.bold(lotteryVoucher)) + interface.Format.newline()
			if lotteryDescription:
				message += '%s%s: %s' % (indent, interface.Translation.string(33040), interface.Format.bold(lotteryDescription)) + interface.Format.newline()
			if lotteryInstruction:
				message += lotteryInstruction
				if not message.endswith('.'): message += '.'
			if lotteryTimeExpiry and lotteryTimeDuration:
				expirationDuration = convert.ConverterDuration(lotteryTimeDuration, convert.ConverterDuration.UnitSecond).string(format = convert.ConverterDuration.FormatWordLong, unit = convert.ConverterDuration.UnitDay)
				expirationTime = convert.ConverterTime(lotteryTimeExpiry, convert.ConverterTime.FormatTimestamp).string(format = convert.ConverterTime.FormatDate)
...
class SpeedTesterGlobal(SpeedTester):
	...
	def __init__(self):
		SpeedTester.__init__(self, name = self.Name, phases = [self.PhaseLatency, self.PhaseDownload, self.PhaseUpload])
		self.mTester = None
		self.mServer = None
		self.mCity = None
...
class SpeedTesterDebrid(SpeedTester):
	...
	def _testLatency(self):
		...
		latencies.sort()
		last = latencies[:self.LatencyCount]
...
class SpeedTesterPremiumize(SpeedTesterDebrid):
	...
	def __init__(self):
		SpeedTesterDebrid.__init__(self, name = self.Name, link = self.Link)
	...
	def _testSelection(self):
		...
		choice = self._testDialog(options)
		if choice == None: return False
		if choice == 0: self.mLinks = [self.LinkServer]
	...
	def _validate(self):
		if not self.mLinks == None and not len(self.mLinks) == 0 and self.mLinks[0] == self.LinkServer or debrid.Premiumize().accountValid():
			return True
		else:
...
class SpeedTesterEasyNews(SpeedTester):
	...
	def __init__(self):
		SpeedTester.__init__(self, name = self.Name, phases = [self.PhaseLatency, self.PhaseDownload])
	...
	def _testSelection(self):
		options = [
			{
				'identifier' : 'europe',
				'name' : 33799,
				'result' : self.LinkEurope,
			},
			{
				'identifier' : 'unitedstateseast',
				'name' : 33797,
				'result' : self.LinkUsEast,
			},
			{
				'identifier' : 'unitedstateswest',
				'name' : 33798,
				'result' : self.LinkUsWest,
			},
		]
		choice = self._testDialog(options)
		if choice == None: return False
		...
		self.mLinkDownload = options[choice]['result'] + self.Download + timestamp
