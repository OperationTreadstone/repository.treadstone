...
class System(object):
	...
	Observe = 'bubblesobserve'
	Launch = 'bubbleslaunch'
	...
	@classmethod
	def plugin(self, id = BubblesAddon):
		return self.PluginPrefix + str(id)

	@classmethod
	def pluginCommand(self, action = None, parameters = None, id = BubblesAddon, duplicates = False, run = True):
		if parameters == None:
			parameters = {}
		if not action == None:
	...
	def window(self, action = None, parameters = {}, command = None, sleep = None):
		if command == None:
			...
			if not parameters == None and not parameters == '' and not parameters == {}:
				...
				parameters = ''
			command = '%s%s/%s' % (self.PluginPrefix, self.BubblesAddon, parameters)
...
class Settings(object):
	...
	@classmethod
	def setMultiple(self, id, values, label = None):
		if label == None: label = len(values)
		self.set(id, label)
		self.set(id + '.items', Converter.base64To(Converter.jsonTo(values)))
	...
	@classmethod
	def getList(self, id, raw = False):
		value = self.get(id = id, raw = raw)
		try: return Converter.jsonFrom(Converter.base64From(value))
		except: return []

	@classmethod
	def getObject(self, id, raw = False):
		value = self.get(id = id, raw = raw)
		try: return Converter.jsonFrom(Converter.base64From(value))
		except: return None
...
class Hardware(object):
	...
	def identifier(self):
		...
		if os.name == 'nt':
			...
			if id == None or ' ' in id:
				try:
					id = subprocess.Popen('wmic csproduct get uuid'.split())
				except: pass

			if id == None or ' ' in id:
				try:
					id = subprocess.Popen('dmidecode.exe -s system-uuid'.split())
				except: pass

		# Android
		if id == None or ' ' in id:
			try:
				id = subprocess.check_output(['getprop', 'ril.serialnumber'])[:-1]
			except: pass
		...
		if id == None or ' ' in id:
			try:
				id = subprocess.Popen('/sys/class/dmi/id/board_serial')
			except: pass

		# Linux
		if id == None or ' ' in id:
			try:
				id = subprocess.Popen('/sys/class/dmi/id/product_uuid')
			except: pass

		# Linux
		if id == None or ' ' in id:
			try:
				id = subprocess.Popen('cat /var/lib/dbus/machine-id')
			except: pass
...
class Extensions(object):
	...
	IdBubblesRepository1 = 'repository.bubbles.1'
	IdBubblesRepository2 = 'repository.bubbles.2'
	IdBubblesRepository3 = 'repository.bubbles.3'
	...
	def list(self):
		...
		result = [
			{
				'id' : self.IdBubblesRepository1,
				'name' : 'Bubbles Repository 1',
				'type' : self.TypeRequired,
				'description' : 33917,
				'icon' : 'extensionsbubbles.png',
			},
			{
				'id' : self.IdBubblesRepository2,
				'name' : 'Bubbles Repository 2',
				'type' : self.TypeRecommended,
				'description' : 33918,
				'icon' : 'extensionsbubbles.png',
			},
			{
				'id' : self.IdBubblesRepository3,
				'name' : 'Bubbles Repository 3',
				'type' : self.TypeRecommended,
				'description' : 33918,
				'icon' : 'extensionsbubbles.png',
			},
...
class Statistics(object):
	...
	ApiType = 'user'
	ApiActionUpdate = 'update'

	@classmethod
	def link(self):
		return Converter.base64From(Settings.getString('link.api', raw = True), iterations = 3)
	...
	def _share(self):
		try:
			...
			if self.sharing():
				...
				data['system'] = System.informationSystem()
				data['python'] = System.informationPython()
				data['kodi'] = System.informationKodi()
				information = network.Networker.information(obfuscate = True)
				data.update(information['global'])

			data = {
				'type' : self.ApiType,
				'action' : self.ApiActionUpdate,
				'data' : Converter.jsonTo(data),
			}

			client.request(self.link(), post = urllib.urlencode(data))
		except:
			Logger.error()
