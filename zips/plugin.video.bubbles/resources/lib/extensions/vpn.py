...
try:
	...
	# When called from RunScript()
	sys.path.append(xbmcaddon.Addon('plugin.video.bubbles').getAddonInfo('path'))
	from resources.lib.extensions import tools
	from resources.lib.extensions import interface
	from resources.lib.extensions import network
...
class Vpn(object):
	...
	def _launchEnabled(self, execution):
		if tools.Settings.getBoolean('general.vpn.enabled'):
			...
			if execution == self.ExecutionKodi and (launch == 1 or launch == 3):
				return True
			if execution == self.ExecutionBubbles and (launch == 2 or launch == 3):
				return True
		return False

	def _launchAutomatic(self):
		identifier = 'BUBBLESVPN'
		enabled = tools.Settings.getBoolean('general.vpn.enabled')
		launch = tools.Settings.getInteger('general.vpn.launch')
		if enabled and (launch == 1 or launch == 3):
			tools.System.automaticInsert(identifier = identifier, command = 'action=vpnLaunch&execution=%s' % self.ExecutionKodi)
		else:
			tools.System.automaticRemove(identifier = identifier)

	def _launch(self, execution):
		script = inspect.getfile(inspect.currentframe())
		tools.System.stopScript(script = script) # Stop if already running.
		time.sleep(3) # Wait for the stop call to finish.
		tools.System.executeScript(script = script, parameters = [execution])

	def launch(self, execution):
		self._launchAutomatic()
...
if __name__ == '__main__':
	if 'vpn.py' in sys.argv[0]:
		execution = urllib.unquote_plus(sys.argv[1])
		Vpn()._execute(execution = execution)
