	...
	def __init__(self, type, currency = tools.Donations.CurrencyNone):
		...
		try:
			...
			if type == self.TypeMini:
				...
				self.addControl(xbmcgui.ControlLabel(x, y, width, height, label, textColor = white, alignment = center, font = 'font12'))
				...
				self.addControl(xbmcgui.ControlButton(x, y, width, height, label, focusTexture = pathFocus, noFocusTexture = pathNormal, alignment = center, textColor = white, font = 'font12'))
				...
				width = 320
				height = 20
				x = self.__centerX(widthTotal) + 440
				y = self.__centerY(heightTotal) + 540
				...
				width = 320
				height = 20
				x = self.__centerX(widthTotal) + 770
				y = self.__centerY(heightTotal) + 540
				label = 'Usenet'
				self.addControl(xbmcgui.ControlLabel(x, y, width, height, label, textColor = white, alignment = center, font = 'font10'))
				...
				height = 220
				x = self.__centerX(widthTotal)
				y = self.__centerY(heightTotal) + 400
				label = tools.System.disclaimer()
				label += ' ' + Translation.string(33911) + ' ' + Translation.string(33913)
				splits = label.split(' ')
				span = 85
				...
				while len(splits) > 0:
					word = splits.pop(0)
					if word == Format.newline():
						label.append(sublabel)
						sublabel = ''
					else:
						sublabel += ' ' + word
						if len(sublabel) > span:
							label.append(sublabel.strip().replace(Format.newline(), ''))
							sublabel = ''
				if not sublabel == '':
					label.append(sublabel)
				...
				try:
					link = network.Networker.linkJoin('%s', 'web', 'resources', 'donations.json')
					data = None

					if data == None:
						try: data = json.loads(urllib.urlopen(link % tools.Settings.getString('link.website', raw = True)).read())
						except: pass

					if data == None:
						try: data = json.loads(urllib.urlopen(link % tools.Settings.getString('link.mirror1', raw = True)).read())
						except: pass

					if data == None:
						try: data = json.loads(urllib.urlopen(link % tools.Settings.getString('link.mirror2', raw = True)).read())
						except: pass
					...
					width = 180
					height = 180
					x = self.__centerX(widthTotal) + 80
					y = self.__centerY(heightTotal) + 370
					self.addControl(xbmcgui.ControlImage(x, y, width, height, donationQrcode))

					width = 860
					height = 200
					x = self.__centerX(widthTotal) + 280
					...
					label = Format.font(donationAddressLabel, bold = True, color = color)
...
class Legal(object):
	...
	def show(self, exit = True):
		while True:
			choice = self._option(message = 35110, left = 33743, right = 33821)
