...
class Provider(object):
			...
			if version == tools.System.version():
				if quick:
					...
					try:
						...
						if force: forcePreset = json.loads(tools.Settings.getString('providers.customization.presets.values%d' % int(forcePreset)).decode('base64'))
					except:
						force = False
					...
					for i in range(len(data)):
						...
						if allow:
							...
							classPointer = imp.load_source(provider['id'], provider['path'])
	...
	@classmethod
	def initialize(self, description = None, enabled = False, forceAll = False, forcePreset = None, progress = False):
		try:
			from resources.lib.extensions import tools
			from resources.lib.extensions import interface
			...
			if not data:
				...
				if progress: total = 175 # +- the total number of providers
				...
				for f in files:
					try:
						...
						type = f[2]
						id = f[4]
						setting = '.'.join(['providers', f[1], type, f[3], id])
						...
						if hasattr(object, 'genre_filter') and isinstance(object.genre_filter, (list, tuple)) and len(object.genre_filter) > 0:
							genres = object.genre_filter

						if hasattr(object, 'base_link'):
							if customLinks and setting in customLinks:
								object.base_link = customLinks[setting]
						...
						if progress:
							total = len(files)
							progressDialog.update(int(40 + (50 * (len(providers) / float(total)))))
				...
				else: data = self._databaseRetrieve(description = description, enabled = enabled, forceAll = forceAll, forcePreset = forcePreset)
	...
	@classmethod
	def provider(self, description, enabled = True, local = True, genres = None):
		description = description.lower().replace(' ', '') # Important for "local library".
		sources = self.providers(description = description, enabled = enabled, local = local, genres = genres)
	...
	@classmethod
	def providers(self, description = None, enabled = True, local = True, genres = None):
	...
	@classmethod
	def providersMovies(self, enabled = True, local = True, genres = None):
		sources = self.providers(enabled = enabled, local = local, genres = genres)
		return [i for i in sources if i['group'] == self.GroupMovies or i['group'] == self.GroupAll]

	@classmethod
	def providersTvshows(self, enabled = True, local = True, genres = None):
		sources = self.providers(enabled = enabled, local = local, genres = genres)
		return [i for i in sources if i['group'] == self.GroupTvshows or i['group'] == self.GroupAll]

	@classmethod
	def providersAll(self, enabled = True, local = True, genres = None):
		sources = self.providers(enabled = enabled, local = local, genres = genres)
		return [i for i in sources if i['group'] == self.GroupAll]

	@classmethod
	def providersTorrent(self, enabled = True, genres = None):
		sources = self.providers(enabled = enabled, genres = genres)
		return [i for i in sources if i['type'] == self.TypeTorrent]

	@classmethod
	def providersUsenet(self, enabled = True, genres = None):
		sources = self.providers(enabled = enabled, genres = genres)
		return [i for i in sources if i['type'] == self.TypeUsenet]

	@classmethod
	def providersHoster(self, enabled = True, genres = None):
		sources = self.providers(enabled = enabled, genres = genres)
		return [i for i in sources if i['type'] == self.TypeHoster]

	@classmethod
	def names(self, enabled = True, local = True, genres = None):
		sources = self.providers(enabled = enabled, local = local, genres = genres)
		...
		try:
			...
			def extract(providers, category):
				...
				for i in range(len(subproviders)):
					subproviders[i]['label'] = interface.Format.bold('[%s %s] ' % (subproviders[i]['category'].capitalize(), subproviders[i]['type'].capitalize())) + subproviders[i]['name']
				return sorted(subproviders, key=lambda k: k['label'])

			interface.Loader.show()

			self.initialize(enabled = False, forceAll = True)
			providers = self.providersAll(enabled = False, local = False)
			items = []
			remaining = []

			items += extract(providers, 'universal')
			items += extract(providers, 'english')
			items += extract(providers, 'german')
			items += extract(providers, 'french')
			items += extract(providers, 'russian')
			items += extract(providers, 'polish')
			items += extract(providers, 'korean')

			labels = [i['label'] for i in items]
			interface.Loader.hide()

			choice = interface.Dialog.select(title = 35167, items = labels)
			if choice >= 0:
				choice = items[choice]
				...
				if link == '' or link == choice['object'].base_link:
					try: del value[choice['setting']]
					except: pass
				else:
					value[choice['setting']] = link
				if not value: value = ''
