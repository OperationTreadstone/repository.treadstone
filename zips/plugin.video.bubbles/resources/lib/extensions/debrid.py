...
class Premiumize(Debrid):
	...
	Services = [
		...
		{	'name' : 'DepFile',				'domain' : 'depfile.com',		 	'limit' : 20,	'cost' : 3	},
		...
		{	'name' : 'FileFactory',			'domain' : 'filefactory.com',		'limit' : 0,	'cost' : 5	},
		...
		{	'name' : 'UploadRocket',		'domain' : 'uploadrocket.com',		'limit' : 20,	'cost' : 3	},
	...
	def _initialize(self):
		...
		def notify():
			...
			apiKey = ''
		...
		try:
			...
			xl = not ord(l[31]) == 99 or not ord(l[35]) == 116 or not ord(l[38]) == 63
	...
	# Delete single item
	def deleteItem(self, id, type = TypeTorrent):
		try:
	...
	def _deleteSingle(self, hashOrLink):
		if network.Networker.linkIs(hashOrLink):
			hash = self.itemId(hashOrLink)
		else:
			hash = hashOrLink

		information = self.itemInformation(hash)
		...
		thread1 = threading.Thread(target = self.deleteItem, args = (id, type))
		thread2 = threading.Thread(target = self.deleteTransfer, args = (hash, type))
		thread1.start()
		thread2.start()
		thread1.join()
		thread2.join()
	...
	def deletePlayback(self, hashOrLink, pack = None):
		try:
			if tools.Settings.getBoolean('accounts.debrid.premiumize.removal'):
	...
	# Delete on failure
	def deleteFailure(self, hashOrLink, pack = None):
		try:
			if tools.Settings.getBoolean('accounts.debrid.premiumize.removal'):
	...
	def addContainer(self, link, title = None, season = None, episode = None):
		try:
			...
			# Already exists. Do not add it a second time, otherwise there are multiple entries in the "My Files" section.
			item = self.item(source['hash'], transfer = False, content = True)
			success = True
			if not item:
				if title == None:
					title = source['name']
			...
			if success: return self._addLink(hash = source['hash'], season = season, episode = episode)
	...
	def itemId(self, link):
		try: return re.search('\/[0-9a-f]{40}\/', link, re.IGNORECASE).group(0).replace('/', '').lower()
		except: return None

	def itemInformation(self, id):
		try:
			id = id.lower()
			items = self._retrieve(category = self.CategoryFolder, action = self.ActionList)
			items = items['content']
			for item in items:
				if item['hash'].lower() == id:
	...
	def _item(self, hash, season = None, episode = None):
		...
		if self.success() and 'content' in result:
			...
			def addDirectory(dictionary, parent = None, parentname = ''):
				id = randomId()
				directory = {}
				...
				if parent == None:
					...
					if directory['name']: parentname += ' ' + str(directory['name'])
				...
				if isinstance(itemsContent, dict):
					for key, value in itemsContent.iteritems():
						if 'type' in value:
							if value['type'] == 'dir':
								sub, largest = addDirectory(value, id, parentname)
								...
								sub = addFile(value, id, parentname)
			...
			def addFile(dictionary, parent, parentname = ''):
				id = randomId()
				file = {}
			...
			if 'content' in result:
				...
				if not season == None and not episode == None:
					...
					for file in allFiles:
						...
						try: fullName = file['parentname'] + ' ' + file['name']
...
class PremiumizeInterface(object):
	...
	def addManual(self):
		...
		if choice >= 0:
			...
			if not link == None or not link == '':
				...
				interface.Loader.show()
				link = self.add(link)
				if network.Networker.linkIs(link):
					choice = interface.Dialog.option(title = 35073, message = 35074)
					if choice: interface.Player.playNow(link)
				result = link
	...
	def add(self, link, title = None, season = None, episode = None, close = True, source = None, pack = False):
		...
		if link == Premiumize.ErrorInaccessible:
			...
			return self._addLink(link, season = season, episode = episode, close = close, pack = pack)

	def _addAction(self, id, type, season = None, episode = None, close = True, pack = False):
		def show(id, season, episode, close, pack):
			items = []
			...
			if choice == 0:
				self._addLink(id, season = season, episode = episode, close = close, pack = pack)
			if choice == 2:
		...
		thread = threading.Thread(target = show, args = (id, season, episode, close, pack))
		thread.start()
	...
	def _addErrorDetermine(self, item, api = False, pack = False):
		...
		if status == Premiumize.StatusError:
			...
			if not 'video' in item or item['video'] == None:
				...
				if pack: message = 'No episode stream found in the pack'
				else: message = 'No useable stream found in the download'
				self._addError(title = title, message = message)
		...
		if error:
			try:
				id = item['hash']
				pack = item['pack'] if 'pack' in item else None
				self.mPremiumize.deleteFailure(id, pack)
			except: pass

		return error

	def _addLink(self, id, season = None, episode = None, close = True, pack = False):
		unknown = 'Unknown'
						...
						while True:
							...
							if (conditionShort or conditionLong) and background:
								...
								if answer:
									...
									self._addAction(id = id, type = type, season = season, episode = episode, close = close, pack = pack)
									return
	...
	def directoryItemAction(self, item):
		try: item = tools.Converter.jsonFrom(item)
		except: pass
		link = item['link']
		...
		if choice >= 0:
			...
			if choice == 'refresh':
				...
				try:
					id = item['id']
					if choice == 'remove':
						self.mPremiumize.deleteSingle(id, wait = True)
						interface.Directory.refresh()
						hide = False # Already hidden by container refresh.
					else:
						item = self.mPremiumize.item(id)
						itemZip = item['link']
	...
	def directoryList(self):
		directory = interface.Directory(content = interface.Directory.ContentAddons)
		items = self.mPremiumize._itemsTransfer()
		itemsNew = [[], [], [], [], [], []]
		...
		for item in items:
			...
			if not status == None and not status == Premiumize.StatusUnknown:
				...
				if status == Premiumize.StatusError:
					...
					color = interface.Format.ColorUltra
					icon = 'downloadsbusy.png'
...
class RealDebrid(Debrid):
	...
	def _request(self, mode, link, parameters = None, httpTimeout = None, httpData = None, httpHeaders = None, httpAuthenticate = True):
		...
		try:
			...
			self.mSuccess = False
			self.mError = 'RealDebrid Unavailable [HTTP Error %s]' % str(error.code)
			self._requestErrors(self.mError, link, httpData, self.mResult)
			try:
			...
			if self.mErrorDescription == 'bad_token' or error.code == 401:
				return redo(mode = mode, link = linkOriginal, parameters = parametersOriginal, httpTimeout = httpTimeout, httpData = httpDataOriginal, httpHeaders = httpHeaders, httpAuthenticate = httpAuthenticate)
...
class RealDebridInterface(object):
	...
	def add(self, link, title = None, season = None, episode = None, close = True, source = None, pack = False):
		...
		if id == RealDebrid.ErrorInaccessible:
			...
			return self._addWait(id = id, season = season, episode = episode, close = close, pack = pack)

	def _addAction(self, id, season = None, episode = None, close = True, pack = False):
		def show(id, season, episode, close, pack):
			items = []
			items.append(interface.Format.font(interface.Translation.string(33077) + ': ', bold = True) + interface.Translation.string(33078))
			items.append(interface.Format.font(interface.Translation.string(33079) + ': ', bold = True) + interface.Translation.string(33080))
			...
			if choice == 0:
				self._addWait(id, season = season, episode = episode, close = close, pack = pack)
			if choice == 2:
	...
	def _addWait(self, id, season = None, episode = None, close = True, pack = False):
		try:
			...
			unknown = 'Unknown'
			title = self.Name + ' Download'
			descriptionInitialize = 'Initializing the download'
			descriptionWaiting = 'Waiting for the download to start '
			descriptionSeeds = 'Waiting for seeds to connect '
			descriptionFinalize = 'Finalizing the download'
			dots = ''
			percentage = 0
			...
			self.tProgressDialog = interface.Dialog.progress(title = title, message = descriptionInitialize, background = background)
			...
			if background: self.tProgressDialog.update(int(percentage), interface.Dialog.title(title), descriptionInitialize)
			else: self.tProgressDialog.update(int(percentage), interface.Format.fontBold(descriptionInitialize))
			...
			while status == RealDebrid.StatusMagnetConversion or status == RealDebrid.StatusFileSelection or status == RealDebrid.StatusQueued:
				...
				dots += '.'
				if len(dots) > 3: dots = ''
				if background: self.tProgressDialog.update(int(percentage), interface.Dialog.title(title), descriptionSeeds + dots)
				else: self.tProgressDialog.update(int(percentage), interface.Format.fontBold(descriptionSeeds + dots))
				...
				if selection == True:
					...
					if status == RealDebrid.StatusFinished: # In case of "cached" RealDebrid torrents that are available immediatley.
						...
						break
				elif selection == RealDebrid.ErrorUnavailable:
					if close: self.tProgressDialog.close()
					title = 'Invalid Stream'
					if pack: message = 'No episode stream found in the pack'
					else: message = 'No useable stream found in the download'
			...
			if choice == 'refresh':
				...
				try:
					...
					if choice == 'remove':
						...
						itemLink = self.mDebrid.add(item['link'])
						if network.Networker.linkIs(itemLink): downloader.Downloader(downloader.Downloader.TypeManual).download(media = downloader.Downloader.MediaOther, link = itemLink)
