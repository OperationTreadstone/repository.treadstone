...
# Bubbles
def getShowCount(indicators, tvdb, limit = False):
	try:
...
def markEpisodeDuringPlayback(imdb, tvdb, season, episode, watched):
	try:
		...
		if int(watched) == 7:
			...
			if allow: trakt.markEpisodeAsWatched(tvdb, season, episode)
		else:
			trakt.markEpisodeAsNotWatched(tvdb, season, episode)
...
def tvshows(tvshowtitle, imdb, tvdb, season, watched):
	...
	try:
		...
		if season == None:
			...
			if int(watched) == 7: trakt.markSeasonAsWatched(tvdb, season)
			else: trakt.markSeasonAsNotWatched(tvdb, season)
		trakt.cachesyncTVShows()
	except:
		pass
