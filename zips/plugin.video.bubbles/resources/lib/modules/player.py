...
class player(xbmc.Player):
	...
	def run(self, title, year, season, episode, imdb, tvdb, url, meta, downloadType = None, downloadId = None, handle = None, source = None):
		try:
			control.sleep(200)
			...
			epsiodeIs = not season == None and not episode == None
			self.type = tools.Media.TypeEpisode if epsiodeIs else tools.Media.TypeMovie

			self.season = int(season) if epsiodeIs else None
			self.episode = int(episode) if epsiodeIs else None
			self.seasonString = '%01d' % self.season if epsiodeIs else None
			self.episodeString = '%01d' % self.episode if epsiodeIs else None
			...
			self.handle = handle
			self.source = source
			self.source['metadata'] = metadata.Metadata.initialize(self.source)
	...
	def getMeta(self, meta):
		...
		try:
			if not self.type == tools.Media.TypeMovie: raise Exception()
		...
		try:
			if not self.type == tools.Media.TypeEpisode: raise Exception()
	...
	def _debridClear(self):
		if self.handle:
			...
			source = self.source['source'].lower()
			hash = self.source['hash'] if ('hash' in self.source and not self.source['hash'] == '') else None
			if source == handler.HandlePremiumize().id() or self.handle == handler.HandlePremiumize().id():
				pack = self.source['pack'] if 'pack' in self.source else None
				link = hash if hash else self.url # Use the resolved link.
				debrid.Premiumize().deletePlayback(link, pack)
			elif source == handler.HandleRealDebrid().id() or self.handle == handler.HandleRealDebrid().id():
				link = hash if hash else self.source['url']
				debrid.RealDebrid().deletePlayback(link)
	...
	def keepPlaybackAlive(self):
		...
		elif self.type == tools.Media.TypeEpisode:
		...
		elif self.type == tools.Media.TypeMovie:'''
		if self.type == tools.Media.TypeMovie:
			while self.isPlayingVideo():
				try:
					self.timeTotal = self.getTotalTime()
					...
					watcher = (self.timeCurrent / self.timeTotal >= .8)
		...
		if progress > 0:
			if self.timeTotal > 0:
				...
				if seconds > 0 and tools.Settings.getInteger('general.playback.resume') == 1:
					...
					resume = interface.Dialog.option(title = 32344, message = label, labelConfirm = 32503, labelDeny = 32501)
					if not resume: seconds = 0
...
class Subtitles:
	...
	def get(self, name, imdb, season, episode, source):
		...
		try:
			if not tools.Settings.getBoolean('subtitles.enabled'):
				return False
			...
			for i in range(len(subtitleIds)):
				subtitleLabels.append(interface.Format.bold(tools.Language.name(subtitleLanguageCodes[i]) + ': ') + subtitleNames[i])
