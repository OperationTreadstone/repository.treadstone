...
class source:
	def __init__(self):
		...
		# [BUBBLESCODE]
		self.enabled = control.setting('accounts.providers.ororo.enabled') == 'true'
		self.user = control.setting('accounts.providers.ororo.user')
		self.password = control.setting('accounts.providers.ororo.pass')
		self.headers = {
		'Authorization': 'Basic %s' % base64.b64encode('%s:%s' % (self.user, self.password)),
		'User-Agent': 'Bubbles for Kodi'
		}
		# [/BUBBLESCODE]
	...
	def sources(self, url, hostDict, hostprDict):
		try:
			...
			sources.append({'source': 'ororo', 'quality': 'HD', 'language': 'en','url': url, 'direct': True, 'debridonly': False, 'memberonly': True})
			# /[BUBBLESCODE]
